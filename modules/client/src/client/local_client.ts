import {Status, IStore, AgentId, BundleLE, BundleBucketLE, BucketParameters, IArrayReplicaOLog, ReplicaId, Output, EReplicaCategoryType, ReplicaType, IReplicaState, BucketId, create_replica_state_id, apply_on_replica} from 'sdb_core'

const enum EFlagDirtyState {
}
...
export class LocalClient {
  ...
  update_or_create(bucket_id: BucketId, replica_id: ReplicaId, o_state: Output<IReplicaState>/*, from: LocalVersion, to: LocalVersion*/) {
    ...
    //TODO test if o_state.data == null
    if (replica_info === null) apply_on_replica(o_state.data, replica_pologs);
    else {
      o_state.data = create_replica_state_id(replica_info.category, replica_info.type, replica_id, bucket_id);
      apply_on_replica(o_state.data, replica_pologs);
    }
  }
